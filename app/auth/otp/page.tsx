    ...
    useEffect(() => {
        if (!call_flag) {
            ...
            if (!code || !state || !username) {
                setMes("Cant find code / state / username param. Will Redirect to login page in 12s.");
                ...
                const verify = async function () {
                    try {
                        let res = await verifyOTPLoginRedirect(state, code, username, window.navigator.userAgent);
                        if (res.code != "Success") {
